...
export default function TaskDetailPanel({ goal, onClose, onEdit, onDelete, onAddChild, completionLocked = false }: Props) {
  ...
  const addNotesImages = async (itemId: string, files: FileList) => {
    const item = goal.checklist.find((c) => c.id === itemId);
    const existing = item?.notesImages ?? [];
    const room = MAX_IMAGES_PER_ITEM - existing.length;
    if (room <= 0) {
      alert(t(lang, "maxImagesReached"));
      return;
    ...
    patchList(
      goal.checklist.map((c) =>
        c.id === itemId ? { ...c, notesImages: [...existing, ...added] } : c,
      ),
    );
  };

  const removeNotesImage = (itemId: string, index: number) => {
    patchList(
      goal.checklist.map((c) => {
        if (c.id !== itemId) return c;
        const existing = c.notesImages ?? [];
        return { ...c, notesImages: existing.filter((_, i) => i !== index) };
      }),
    );
  };
  ...
  return (
    ...
    >
              ...
              <div className="mt-4 space-y-3">
                {goal.checklist.map((item) => (
                  <TaskItem
                    ...
                    onOpenNotesEditor={() => setNotesEditorItemId(item.id)}
      ...
      {/* Notes modal — same window & image-upload behavior as the completion-method modal */}
      {notesEditorItemId && (() => {
        ...
        return (
          <NotesModal
            item={item}
            lang={lang}
            onChangeNote={(note) => updateItemNotes(item.id, note)}
            onAddImages={(files) => addNotesImages(item.id, files)}
            onRemoveImage={(idx) => removeNotesImage(item.id, idx)}
        ...
        <div className="flex items-center gap-1 opacity-70 transition-opacity duration-150 hover:opacity-100">
          <button
            ...
            className={`relative rounded-md p-1.5 transition-colors duration-150 hover:bg-terrace-500/10 hover:text-terrace-600 ${
              item.notes || (item.notesImages?.length ?? 0) > 0 ? "text-terrace-600" : "text-ink-soft"
            }`}
          >
            <StickyNote size={15} />
            {(item.notes || (item.notesImages?.length ?? 0) > 0) && (
...
function NotesModal({
  ...
  const handleClose = () => {
    if (draft !== (item.notes ?? "")) onChangeNote(draft);
    onClose();
  };
