...
import type { Goal, ChecklistItem, TaskNote, TimeSession } from "../types";
...
export default function TaskDetailPanel({ goal, onClose, onEdit, onDelete, onAddChild, completionLocked = false }: Props) {
  ...
  const { updateGoal, updateGoalAndFlush, effProgress, lang, members, goals } = useStore();
  ...
  const migrateNotesIfNeeded = (itemId: string) => {
    const item = goal.checklist.find((c) => c.id === itemId);
    if (!item) return;
    if ((item.notesList?.length ?? 0) > 0) return;
    if (!item.notes && (item.notesImages?.length ?? 0) === 0) return;
    const migrated: TaskNote = {
      id: uid(),
      title: t(lang, "notesModalTitle"),
      body: item.notes ?? "",
      images: item.notesImages ?? [],
      createdAt: new Date().toISOString(),
    };
    patchList(
      goal.checklist.map((c) => (c.id === itemId ? { ...c, notesList: [migrated] } : c)),
    );
  };

  const saveNotesList = (itemId: string, notesList: TaskNote[]) => {
    // flush immediately (bypassing the usual debounce) so the notes survive
    // an instant page refresh right after saving
    updateGoalAndFlush(goal.id, {
      checklist: goal.checklist.map((c) => (c.id === itemId ? { ...c, notesList } : c)),
    });
  };
  ...
  return (
    ...
    >
              ...
              <div className="mt-4 space-y-3">
                {goal.checklist.map((item) => (
                  <TaskItem
                    ...
                    onOpenNotesEditor={() => {
                      migrateNotesIfNeeded(item.id);
                      setNotesEditorItemId(item.id);
                    }}
      ...
      {/* Notes modal — multi-note list + editor for this checklist item */}
      {notesEditorItemId && (() => {
        const item = goal.checklist.find((c) => c.id === notesEditorItemId);
        if (!item) return null;
        return (
          <NotesModal
            notesList={item.notesList ?? []}
            lang={lang}
            onSave={(list) => saveNotesList(item.id, list)}
            onClose={() => setNotesEditorItemId(null)}
          />
        );
      })()}
...
function TaskItem({
  ...
  const hasNotesContent =
    !!item.notes ||
    (item.notesImages?.length ?? 0) > 0 ||
    (item.notesList ?? []).some((n) => n.title || n.body || n.images.length > 0);
  const itemElapsedMs =
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex items-center gap-1 opacity-70 transition-opacity duration-150 hover:opacity-100">
          <button
            onClick={onOpenNotesEditor}
            title={t(lang, "itemNotes")}
            className={`relative rounded-md p-1.5 transition-colors duration-150 hover:bg-terrace-500/10 hover:text-terrace-600 ${
              hasNotesContent ? "text-terrace-600" : "text-ink-soft"
            }`}
          >
            <StickyNote size={15} />
            {hasNotesContent && (
              <span className="absolute end-0.5 top-0.5 h-1.5 w-1.5 rounded-full bg-terrace-600 ring-1 ring-card" />
            )}
          </button>
          <button
